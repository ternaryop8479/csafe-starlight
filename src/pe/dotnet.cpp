...
namespace {
	...
	bool contains_marker(const uint8_t *data, size_t size, const char *marker) {
		...
		if (marker_len == 0 || marker_len > size)
			return false;
	...
	bool compute_methoddef_layout(const std::array<uint32_t, 45> &rows, uint8_t heap_sizes, size_t tables_base, MethodDefLayout &out) {
		...
		auto ssize = [&](uint32_t table) {
			return rows[table] < 65536 ? size_t(2) : size_t(4);
		};
		auto csize = [](uint32_t max_rows) {
			return max_rows < 16384 ? size_t(2) : size_t(4);
		};
...
DotnetFeatPack extract_dotnet_feats(const PeView &view) {
	...
	if (clr_dir.rva == 0 || clr_dir.size < 16)
		return feats;

	size_t clr_offset = 0;
	if (!view.rva_to_offset(clr_dir.rva, clr_offset) || clr_offset + 16 > view.size())
		return feats;
	...
	if (!view.rva_to_offset(metadata_rva, metadata_offset) || metadata_offset + 20 > view.size())
		return feats;
	if (read<uint32_t>(data, metadata_offset, view.size()) != 0x424A5342)
		return feats; // BSJB
	...
	if (version_end + 4 > view.size() || version_end > metadata_offset + metadata_size_clamped)
		return feats;
	...
	for (uint16_t i = 0; i < stream_count && stream_header + 8 <= view.size(); ++i) {
		...
		stream_header += 8;
		while (stream_header < view.size() && data[stream_header] != 0)
			stream.name.push_back(static_cast<char>(data[stream_header++]));
		if (stream_header >= view.size())
			return feats;
		++stream_header;
		while ((stream_header - (version_end + 4)) % 4 != 0)
			++stream_header;
		if (stream.offset > metadata_size_clamped || stream.size > metadata_size_clamped - stream.offset)
			continue;
	...
	for (const Stream &stream : streams) {
		...
		if (stream.name == "#Strings") {
			strings = ptr;
			strings_size = stream.size;
		} else if (stream.name == "#US") {
			user_strings = ptr;
			user_strings_size = stream.size;
		} else if (stream.name == "#Blob") {
			blob = ptr;
			blob_size = stream.size;
		} else if (stream.name == "#GUID") {
			guid_size = stream.size;
		} else if (stream.name == "#~" || stream.name == "#-") {
			tables = ptr;
			tables_size = stream.size;
		}
	}
	...
	if (strings != nullptr)
		feats.strings_entropy = entropy(strings, strings_size);
	if (user_strings != nullptr)
		feats.user_strings_entropy = entropy(user_strings, user_strings_size);
	(void)resource_rva;
	(void)resource_size;
	...
	if (strings != nullptr) {
		...
		while (off < strings_size) {
			...
			size_t len = 0;
			while (len < remaining && strings[off + len] != 0)
				++len;
			if (len == 0) {
				++off;
				continue;
			...
			if (entropy(strings + off, len) > 4.5)
				++high_entropy_cnt;
			off += len + 1;
		}
	...
	if (user_strings != nullptr) {
		...
		while (off < user_strings_size) {
			...
			if (used == 0 || byte_len > user_strings_size - off - used)
				break;
	...
	if (tables == nullptr || tables_size < 24)
		return feats;
	...
	for (size_t table = 0; table < 64; ++table) {
		if ((valid & (uint64_t(1) << table)) == 0)
			continue;
		if (rows_offset + 4 > tables_size)
			return feats;
	...
	if (rows[6] > 0) {
		...
		if (compute_methoddef_layout(rows, heap_sizes, rows_offset, layout) && layout.row >= 8 && layout.offset < tables_size
			...
			for (uint32_t i = 0; i < rows[6]; ++i) {
				...
				size_t body_off = 0;
				if (!view.rva_to_offset(rva, body_off) || body_off + 1 > view.size()) {
					++missing;
					continue;
				}
