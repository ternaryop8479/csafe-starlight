...
std::vector<ImportEntry> enumerate_imports(const PeView &view) {
	...
	if (dir.rva == 0 || dir.size < 20) return result;
	size_t descriptor_offset = 0;
	if (!view.rva_to_offset(dir.rva, descriptor_offset)) return result;
	...
	for (size_t d = descriptor_offset; d + 20 <= dir_end; d += 20) {
		...
		if (original_thunk == 0 && name_rva == 0 && first_thunk == 0) break;
		size_t name_offset = 0;
		if (!view.rva_to_offset(name_rva, name_offset) || name_offset >= total) continue;
		...
		if (!view.rva_to_offset(thunk_rva, thunk_offset)) continue;
