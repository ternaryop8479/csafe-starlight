...
constexpr SIZE_T kDotnetFeatDims = 41; ///< CLR与.NET元数据特征维度(DotnetFeatPack)
...
constexpr SIZE_T kTotalFeatDims = 263; ///< 总特征维度(36 + 35 + 91 + 16 + 2 + 22 + 41 + 8 + 12)

/**
 * @brief 将FeatPack按固定顺序序列化为double特征向量
 * @note 特征顺序约定: 先EFG特征(36维), 再TSPM特征(35维), 然后PE特征(91维), 再分块熵特征(16维), 签名置信度特征(2维), Rich Header特征(22维), .NET特征(41维), IAT特征(8维), 最后能力类别特征(12维).
